...
`samples.csv` 记录配比和布局。通用记录表 `measurements.csv` 支持仪器读数和由相机图像
导出的结果，除 ID 和观察模式外有意留空。它与下文的仪器专用记录表都不会读取 RAW 图像，
也不会自动导入测量结果。
...
每次生成还会写入 `measurements-i1pro2.csv`、`backings.csv`、
`backing-references-i1pro2.csv`，以及两种语言的仪器指南：
`measurements-i1pro2-guide.txt`（英文）和
`measurements-i1pro2-guide.zh-CN.txt`（简体中文）。
两份指南分别复制自纳入版本控制的
[英文仪器规程模板](templates/measurements-i1pro2-guide.txt)和
[简体中文仪器规程模板](templates/measurements-i1pro2-guide.zh-CN.txt)；
改进共享工作流程时，应修改这些源模板，而不是生成的副本。

针对当前的 107 个试片，仪器记录表预留了 642 次读数：
先进行白底反射测量，再进行黑底反射测量，每个试片在每种底衬上重新定位测量三次。
每个 `reading_id` 都唯一，例如 `TC01-W-01`。记录表包含设计元数据以便交叉核对，
这些数据不是实测挤出比例。通用记录表与仪器记录表不会相互同步；
采用本工作流程时应使用仪器记录表，而不是重复录入两份结果。

* 填入实际的 `L_star`、`a_star` 和 `b_star` 值，保留数值正负号和采集精度。
  空白表示尚未测量，不是零。
* 只有导出该次读数的光谱后，才填写 `spectral_file`。
  保留原始波长标签和单位；只记录 Lab 会丢失光谱。
* 根据实际设置核对预填的 D50 / CIE 1931 2 度观察者、顶面和 0 度旋转角度。
  这些是规程默认值，不是实际观测记录。
* 记录实际厚度、打印批次、测量会话、底衬、采集软件及版本、仪器身份和测量条件。
  不要将设计厚度复制到用于填写物理实测值的字段中。
* 生成器将 `measurement_condition` 留空。单次实验可以为尚未测量的行预填选定的
  原生测量条件，但这只是规程默认值：必须核对实际采集条件，绝不能重新标记已有结果。
  Lab 计算使用 D50，并不意味着采集时使用了原生 M1 或 M2 条件。
  ArgyllCMS 文档说明其 i1Pro 2 驱动不使用设备的 UV 模式；
  应明确标注模拟条件或荧光增白剂（FWA）补偿，而不能将其报告为原生硬件条件。

使用仪器原配校准底座校准仪器，并使用独立且已明确标识的样品底衬。
在试片中央区域进行点测量，确保支撑稳定、仪器放置方式可重复。
不要压缩薄试片，也不要跨越不同高度的试片扫描。求平均值之前先保存每次独立读数；
改变朝向和重新打印的试片需要分别记录，并为读数分配唯一标识。
反射光谱包含样品、底衬和几何条件共同产生的响应，
并不代表材料本征吸收或总透射率的测量结果。
背光测量需要单独制定规程，不包含在这份记录表中。

**裸底衬对照：** `backings.csv` 记录每块底衬的材料、批次、
纸张叠层、厚度、表面处理、使用面和下垫材料。
WB01 和 BB01 是默认的白／黑底衬标识，并非经过认证的光学标准。
试片记录表预填了这些 ID；应核对实际设置，设置改变时使用新的 ID。
`backing-references-i1pro2.csv` 单独预留了 12 次尚未测量的对照读数：
两种底衬 x 一次测量会话的开始／结束 x 三次重复。
在试片行和对照行中填写相同的 `measurement_session`；
既要匹配会话和 `backing_id`，也要匹配采集条件和色度计算条件。

对照表不包含试片 ID 或耗材百分比。不要将它用于配方拟合或留出集评估，
也不要用试片 Lab 减去底衬 Lab。保留每次独立读数的光谱和放置信息。
后续测量会话或均匀性检查应追加具有唯一标识的对照读数，而不是替换已有读数。
各字段的含义见仪器规程。

对于预算受限、使用复印纸的实验，在固定底衬上进行原生 M2 测量，
可以在不购买认证白板的情况下支持经验校准。
记录实际纸张叠层及裸底衬光谱；M2 会减少紫外激发的荧光，
但不会使纸张变成中性或完全不产生荧光的材料。
试片和两种裸底衬必须使用相同的采集条件。
将 M0/M1 和模拟条件分别保存，不要减去底衬 Lab，也不要将底衬强制归一为理想白色。
更换底衬或光照后，应单独验证。

### 校准产物

将可复现的源文件、本地实验和已发布的测量数据分开保存：
...
生成器要求输出目录尚不存在，绝不会原地刷新已有实验。
归档已有实验时，应将整个实验目录复制到新的 `calibration/runs/<run-id>/`，
在确认副本无误之前保留原件。保留 `thickness-cmyn.3mf` 等手动创建的项目；
当前生成器不会输出该文件。重新生成的项目不能替代实际用于打印已测量样品的项目或 G-code。

将单次实验的模型、布局、截图、原始采集文件、设置、测量导出文件和验证记录保存在一起。
副本不会相互同步：应选定一个工作副本用于后续读数。
副本校验和或历史验证报告只描述当时的快照，并不能代表未来修改、
实物颜色准确度或新版生成器的情况。Git 忽略的实验目录不会由 Git 备份；应另行保留备份。

只有获得真实测量数据后，才创建数据集目录。
将选定读数连同原始光谱、精确的 `samples.csv` 映射、实验清单、
实际打印／测量条件和耗材批次一并纳入数据集。
如果已知生成器／模板版本，应记录该版本，并在关联归档中保留实际打印所用的
完整项目和 G-code；不要猜测缺失的来源信息，也不要用当前生成器重建历史映射。
保留留出集标签和原始的逐次读数，使验证保持独立，并能够评估重复性。

发布前应审查数据集路径、仪器序列号和第三方再分发权利。
只纳入相关测量数据及来源信息，而不是整个工作目录。
大型照片和可打印项目包可保存在单独的归档或发布附件中。
空白记录表是生成产物，不是测量数据集。CSV 和 JSON 没有被全局忽略，
因此整理后的数据集可以正常纳入版本控制。
